...
correlation_network_rev <- function(X, sel_features, title, options = list()) {
  ...
  if (!("FDR" %in% names(options))) {
    options$FDR <- 0.05 # no filter based on signifance
  }
  if (!("text_size" %in% names(options))) {
    options$text_size <- 4.5
  }
  ...
  node_colors <- vector(mode = "list", length = length(igraph::V(graph_cors)$name))
  node_colors[is.element(igraph::V(graph_cors)$name, sel_features)] <- 'gray' # selected features
  node_colors[!is.element(igraph::V(graph_cors)$name, sel_features)] <- 'white' # other features
  
  plt_graph <- ggraph::ggraph(layout) +
    ggraph::geom_edge_link(aes(color = r), edge_width = 1.2) +
    ggplot2::guides(edge_alpha = 0.2, edge_width = "none") +
    ...
    ggraph::geom_node_point(shape = 21, color = "gray",
                            fill = node_colors, size = options$node_size, stroke = 0.1) +
