...
#' @export
#'
visualize_ropls_scores_revs <- function(model, y, options = list()) {
  ...
  plt_scores <- ggplot2::ggplot(df_scores, ggplot2::aes(LV1, LV2, fill = y)) +
    ...
    ggplot2::scale_fill_manual(values = c("red2", "royalblue2"), labels = c("naive", "SIVmac239_∆GY"))+
  ...
  if (is.factor(y)) {
    plt_scores <- plt_scores +
      ...
      ggplot2::scale_fill_manual(values = c("red3", "royalblue3"), labels = c("naive", "SIVmac239_∆GY"))+
      #ggplot2::scale_color_manual(values = options$colors[[y_name]])
      ggplot2::scale_color_manual(values = c("red3", "royalblue3"))
  } else {
    plt_scores <- plt_scores +
