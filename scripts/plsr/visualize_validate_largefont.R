...
#' @return plot handle
#' @export
visualize_validate_largefont <- function(vals, options = list()) {
  ...
  plt <- ggplot2::ggplot(df_val, ggplot2::aes(x = model, y = score), fill = "gray") +
    ...
    ggplot2::stat_summary(fun.data = data_summary, geom = "pointrange", size = 1, fatten = 1, color = "red") +
    ggplot2::theme_classic() +
    ggplot2::theme(legend.position = "none",
                   axis.text = ggplot2::element_text(color = "black", size = 26),
                   axis.title = ggplot2::element_text(color = "black", size = 26),
                   axis.text.x =  ggplot2::element_text(size = 26, angle = 0, hjust = 0.5)) +
    ggplot2::ylab(options$y_label) +
    ggplot2::scale_x_discrete("", labels = x_labels)
  
  if ("rf" %in% levels(df_val$model)) {
    plt <- plt +  ggpubr::geom_bracket(xmin = 1, xmax = which(levels(df_val$model) == "rf"),
                                       inherit.aes = FALSE, label.size = 7,
